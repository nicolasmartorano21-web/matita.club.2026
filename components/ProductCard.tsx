...
const getImgUrl = (id: string | undefined, w = 400) => {
  ...
  // Usamos q_auto y f_auto para máxima compatibilidad y velocidad
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[1000] flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm animate-fadeIn overflow-hidden">
          <div 
            className="bg-white w-full max-w-5xl h-[94vh] md:h-auto md:max-h-[90vh] rounded-t-[2.5rem] md:rounded-[3.5rem] shadow-2xl relative flex flex-col md:flex-row overflow-hidden transition-transform animate-slideUp"
            onClick={(e) => e.stopPropagation()}
          >
            
            {/* BOTÓN CERRAR - Diseño unificado y visible */}
            <button 
              onClick={() => setShowModal(false)} 
              className="absolute top-4 right-4 z-[1020] w-12 h-12 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-xl active:scale-90 transition-all border-2 border-gray-100"
            >
            ...
            <div className="flex-grow overflow-y-auto scrollbar-hide flex flex-col md:flex-row pb-32 md:pb-0">
              
              {/* IZQUIERDA: IMAGEN (Aspect Square garantizado) */}
              <div className="w-full md:w-1/2 bg-white shrink-0">
                <div className="w-full aspect-square bg-[#fdfaf6] flex items-center justify-center p-6 md:p-12 relative">
                  <img 
                    key={activeImage}
                  ...
                  {/* Mini-galería flotante en imagen */}
                  {productImages.length > 1 && (
                    <div className="absolute bottom-4 left-0 w-full px-4 flex justify-center gap-2 overflow-x-auto scrollbar-hide">
                      {productImages.map((img, i) => (
                        <button 
                          key={i} 
                          onClick={() => setActiveImage(i)} 
                          className={`w-12 h-12 rounded-xl border-2 transition-all shrink-0 shadow-sm ${
                            activeImage === i ? 'border-[#f6a118] scale-105' : 'border-white opacity-60'
                          }`}
                        >
              ...
              <div className="w-full md:w-1/2 p-6 md:p-12 flex flex-col bg-[#fdfaf6]">
                <div className="mb-6">
                   ...
                   <h2 className="text-3xl md:text-5xl font-bold text-gray-800 leading-none mb-4 uppercase tracking-tighter">{product.name}</h2>
                   <div className="bg-white/70 p-5 rounded-[1.5rem] border-2 border-white shadow-sm">
                     <p className="text-base text-gray-500 italic leading-relaxed font-matita">
                       {product.description || 'Este producto fue seleccionado por Matita para llenar de magia tu día a día.'}
                     </p>
                   </div>
                </div>

                <div className="mb-8">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-3">Elige tu variante:</p>
                  <div className="grid grid-cols-2 gap-2">
                    {(product.colors || []).map(c => (
                      <button 
                        key={c.color} 
                        onClick={() => c.stock > 0 && setSelectedColor(c.color)} 
                        className={`relative p-3 rounded-2xl border-4 transition-all flex flex-col items-center justify-center ${
                          c.stock <= 0 
                            ? 'bg-gray-50 text-gray-200 border-transparent opacity-50 cursor-not-allowed' 
                            : selectedColor === c.color 
                              ? 'bg-white border-[#f6a118] shadow-md' 
                              : 'bg-white border-white hover:border-gray-50'
                        }`}
                      >
                        <span className={`text-sm font-bold uppercase ${selectedColor === c.color ? 'text-[#f6a118]' : 'text-gray-700'}`}>{c.color}</span>
                        <span className="text-[9px] font-bold opacity-30 uppercase tracking-tighter">Quedan: {c.stock}</span>
                        {selectedColor === c.color && (
            ...
            {/* PIE DE PÁGINA: FIJO (Sticky en móvil simulado con Absolute) */}
            <div className="absolute bottom-0 left-0 w-full p-5 md:p-10 bg-white/95 backdrop-blur-md border-t-2 border-gray-100 rounded-t-[2.5rem] md:rounded-none shadow-[0_-10px_30px_rgba(0,0,0,0.1)] z-[1010]">
              <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex justify-between items-center w-full md:w-auto md:gap-10">
                  <div className="flex flex-col">
                    ...
                    <p className="text-[10px] text-[#ea7e9c] font-bold mt-1 uppercase tracking-widest">Sumas {product.points || 0} pts Matita ✨</p>
                  </div>
                  <div className="text-right md:text-left">
                    <p className="text-[10px] text-gray-300 font-bold uppercase tracking-widest leading-none">Seleccionado</p>
                    <p className="text-lg font-bold text-[#f6a118] uppercase truncate max-w-[100px]">{selectedColor || '---'}</p>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className={`w-full md:flex-grow py-4 md:py-6 rounded-full text-xl md:text-3xl font-bold shadow-xl active:scale-95 transition-all uppercase tracking-tighter border-4 border-white ${
                    currentStock <= 0 
                      ? 'bg-gray-200 text-gray-400 cursor-not-allowed' 
                      : 'matita-gradient-orange text-white hover:shadow-2xl'
                  }`}
                >
